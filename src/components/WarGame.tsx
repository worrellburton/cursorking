  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      for (const beam of state.healBeams) {
        ...
        grad.addColorStop(0.5, "rgba(68, 255, 136, 0.4)");
      ...
      for (const p of state.players) {
        ...
        ctx.save();
        if (p.role === "orb") {
          // Orb: translucent shield
          const orbGrad = ctx.createRadialGradient(px, py, 0, px, py, r);
          orbGrad.addColorStop(0, color + "40");
          orbGrad.addColorStop(0.7, color + "20");
          orbGrad.addColorStop(1, color + "60");
        ...
        if (p.role === "sniper" && p.charging > 0) {
          ...
          const chargeColor = p.charging >= 1 ? "#ff4444" : "#ffaa00";
        ...
        // Health bar
        const hpBarW = Math.max(30, r * 2.5);
        const hpBarH = 4;
      ...
      // Kill feed (top right)
      const now = Date.now();
      const visibleKills = killFeedRef.current.filter(k => now - k.time < 5000);
      ctx.save();
      for (let i = 0; i < visibleKills.length; i++) {
        ...
        const text = `${killer?.name ?? "?"} ⚔ ${victim?.name ?? "?"} (${k.victimRole.toUpperCase()})`;

        ctx.globalAlpha = 1 - age;
        ctx.font = "bold 11px 'Inter', sans-serif";
        ctx.textAlign = "right";
        ctx.fillStyle = TEAM_COLOR[k.victimTeam === "top" ? "bottom" : "top"];
        ctx.fillText(text, W - 16, 30 + i * 18);
      }
      ctx.restore();
      ...
      if (myPlayer) {
        ...
        ctx.shadowColor = TEAM_COLOR[myPlayer.team];
        ctx.shadowBlur = 8;
        ctx.fillText(
      ...
      ctx.fillText(`TOP: ${topAlive}`, W / 2 - 60, 28);
      ctx.fillStyle = "rgba(255,255,255,0.3)";
      ctx.fillText("vs", W / 2, 28);
      ctx.fillStyle = TEAM_COLOR.bottom;
      ctx.fillText(`BOT: ${botAlive}`, W / 2 + 60, 28);
      ctx.restore();
      ...
      if (state.winner) {
        ...
        const text = winnerIsMyTeam ? "VICTORY!" : "DEFEAT";
        ...
        ctx.shadowBlur = 0;
        ctx.fillText(`${state.winner.toUpperCase()} TEAM WINS`, W / 2, H / 2 + 20);

        ctx.font = "13px 'Inter', sans-serif";
        ctx.fillStyle = "rgba(255,255,255,0.3)";
        ctx.fillText("Returning to lobby...", W / 2, H / 2 + 50);
        ctx.restore();
      }
